...
public class ShortestPathFromSrcToDest {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < dist.length; i++) {
            dist[i] = new int[]{Integer.MAX_VALUE, -1};
        }
        dist[0] = new int[]{0, 0};
        ...
        while (!p.isEmpty()) {
            ...
            for (Edge i : graph.get(node[1])) {
                if ((node[0] + i.w) < dist[i.i][0]) {
                    dist[i.i][0] = node[0] + i.w;
                    dist[i.i][1] = node[1];
                    p.add(new int[]{dist[i.i][0], i.i});
                }
            }
        }
        ...
        for (int i = 0; i < dist.length; i++) {
            System.out.println("For node " + i + " distance  = " + dist[i][0] + " from " + dist[i][1]);
        }
    }
}
